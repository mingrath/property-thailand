        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {propertyTypes.map(({ type, icon: Icon, count }) => (
            <Link
              ...
              className="group flex flex-col items-center gap-3 p-6 rounded-2xl border border-gray-100 hover:border-brand-gold hover:shadow-lg transition-all"
            >
              <Icon className="w-8 h-8 text-brand-slate group-hover:text-brand-gold transition-colors" />
            ...
            {[
              ...
              >
                <div className="aspect-[4/3] bg-gray-100 relative">
                  ...
                  <span className="absolute top-3 right-3 bg-white/90 text-brand-navy text-xs font-medium px-2.5 py-1 rounded-full">
                    {t("forSale")}
                  </span>
                </div>
                <div className="p-5">
                  <div className="text-2xl font-bold text-brand-navy mb-1">{item.price}</div>
                  <h3 className="font-medium text-gray-700 mb-2 group-hover:text-brand-gold transition-colors">
                    {item.title}
                  </h3>
